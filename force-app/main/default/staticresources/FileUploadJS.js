     ...
     * @date   2016-06-17
     * @description Executes when start Upload button is selected
    */
     ...
     * @author Mateusz Przywara
     * @date   2016-06-17
     * @description If elements with status pending exist. Trigger upload. Disable buttons. On the end change status
     to complete. If there is nothing to upload. Enable buttons.
    */
     ...
     * @author Mateusz Przywara
     * @date   2016-06-17
     * @description Uploads a chunk of bytes, if attachmentId is passed in it will attach the bytes to an existing attachment record
    */
		...
		if (byteChunkArray.length > 0){
			...
			reader.onloadend = function(evt) {
				if (evt.target.readyState == FileReader.DONE) { //Make sure read was successful, DONE == 2
					...
					BTN_DragNDropComponentController.attachBlob(parentId,attachmentId,currentFile.name,currentFile.type,base64value,function(result,event){
